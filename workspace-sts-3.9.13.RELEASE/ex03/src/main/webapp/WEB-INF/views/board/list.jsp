...
document.addEventListener("DOMContentLoaded", () => {
	...
	function checkModal(result) {
		...
		if (parseInt(result) > 0) {
			document.querySelector('.modal-body').innerHTML = `게시글 \${parseInt(result)} 번이 등록되었습니다.`
		}
	...
	document.querySelectorAll('.paginate_button a').forEach((element) => {
		element.addEventListener('click', (e) => {
			e.preventDefault()
			console.log('click')
			actionForm.querySelector('input[name="pageNum"]').value = element.attributes.href.value
			actionForm.submit()
		})
	})
	...
	document.querySelectorAll('.move').forEach((element) => {
		element.addEventListener('click', (e) => {
			e.preventDefault()
			actionForm.insertAdjacentHTML('beforeend', `<input type="hidden" name="bno" value="\${element.attributes.href.value}">`)
	...
	document.querySelector('#searchForm button').addEventListener('click', (e) => {
		e.preventDefault()
		if (!searchForm.querySelector('select').selectedOptions[0].value) {
			alert('검색의 종류를 선택하세요')
			return false
		}
